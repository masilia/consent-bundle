...
class CookieAdminController extends AbstractController
{
    ...
    public function create(Request $request, CookieCategory $category): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->notificationHandler->success(sprintf('Cookie "%s" has been created.', $cookie->getName()));
        } else {
    ...
    public function edit(Request $request, Cookie $cookie): Response
    {
        ...
        if ($cookie->isAutoGenerated()) {
            $this->notificationHandler->error('Cannot edit auto-generated cookies from third-party services.');
            return $this->redirectToRoute('masilia_consent_admin_policy_view', [
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $this->cookieRepository->save($cookie, true);
            $this->notificationHandler->success(sprintf('Cookie "%s" has been updated.', $cookie->getName()));
        } else {
            foreach ($form->getErrors(true) as $error) {
                $this->notificationHandler->error($error->getMessage());
            }
    ...
    public function delete(Request $request, Cookie $cookie): Response
    {
        ...
        if ($cookie->isAutoGenerated()) {
            $this->notificationHandler->error('Cannot delete auto-generated cookies from third-party services.');
        ...
        if ($this->isCsrfTokenValid('delete' . $cookie->getId(), $request->request->get('_token'))) {
            ...
            $this->notificationHandler->success(sprintf('Cookie "%s" has been deleted.', $name));
        }
